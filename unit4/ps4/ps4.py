...
def generate_models(x, y, degs):
    ...
    fits = []
    for deg in degs:
        fits.append(np.array(np.polyfit(x, y, deg)))
    return fits
...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.title('Measured temperatures')
    pylab.xlabel('years')
    pylab.ylabel('temperature in celcius')
    for model in models:
        yEst = np.polyval(model, x)
        pylab.plot(x, yEst, color[i],
                   label = str(len(model)-1) 
                   +'-dimension poly, R2 = ' 
                   +str(r_squared(y, yEst)))
        i += 1
...
function = generate_models([1961, 1962, 1963],[4.4,5.5,6.6],[1, 2])[0]
est = function[0]*np.array([1961, 1962, 1963]) + np.array(function[1])
